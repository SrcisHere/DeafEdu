      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               ...
               $select_educators = $conn->prepare("SELECT * FROM `educators` WHERE id = ?");
               $select_educators->execute([$fetch_contents['educator_id']]);
               $fetch_educator = $select_educators->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="box">
      <div class="educator">
